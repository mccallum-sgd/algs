...
public class MyBST<Key extends Comparable<Key>, Value> {
    ...
    public int avgPathLength() {
        int count = 0;
        return allDepths(root, count)/size() + 1;
    }
    
    private int allDepths(Node curr, int count) {
        if (curr.hasLeft() && curr.hasRight())
            return count + 
                    allDepths(curr.left, count++) + 
                    allDepths(curr.right, count++);
    ...
    ***************************************************************************/
    ...
    public static void main(String[] args) {
        ...
        for (int N = 100; N <= 12800; N*=2) {
            int[] mt = new int[ITR];
            for (int i = 0; i < ITR; i++) {
                MyBST<Integer, Integer> t = new MyBST<Integer, Integer>();
                for (int j = 0; j < N; j++)
                    t.put(StdRandom.uniform(N), StdRandom.uniform(N));
                mt[i] = t.avgPathLength();
            }
